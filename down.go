...
func Down(db GooseDB, dir string, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
func DownTo(db GooseDB, dir string, version int64, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
// target version.
func downToNoVersioning(db GooseDB, migrations Migrations, version int64) error {
